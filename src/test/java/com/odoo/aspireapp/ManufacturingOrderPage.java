...
public class ManufacturingOrderPage extends BaseTest {
	...
	public void beforeClass(String browserName, String urlPage) {
		...
		log.info("Pre-Condition - Step 01: Verify 'Aspire' logo displayed");
		verifyTrue(loginPage.isAspireAppLogoDisplayed());

		log.info("Pre-Condition - Step 02: Log-in to web application with ");
		...
		log.info("Pre-Condition - Step 03: Verify menu navigation bar displayed");
		verifyTrue(homePage.isMenuNavbarDisplayed());

		log.info("Pre-Condition - Step 04: Navigate to 'Inventory' page");
		...
		log.info("Pre-Condition - Step 15: Click 'Update Quantity' button");
		productPage.clickOnButton(driver, "Update Quantity");

		log.info("Pre-Condition - Step 16: Get nocontent view displayed: 'Your stock is currently empty'");
		verifyTrue(productPage.isMessageOnUpdateQuantityDisplayed(driverData.getEmptyStockOnQuanityPage()));

		log.info("Pre-Condition - Step 17: Click 'Create' button");
		productPage.clickOnButton(driver, "Create");

		log.info("Pre-Condition - Step 18: Enter value into 'Counted Quantity' textbox is larger than 10");
		productPage.updateQualityOfNewProductIsMoreThan10(productName);

		log.info("Pre-Condition - Step 19: Click 'SAVE' button");
		productPage.clickOnButton(driver, "Save");

		log.info("Pre-Condition - Step 20: Click 'Home' menu");
	...
	public void TC_01_Create_A_Manufacturing_Order() {
		...
		log.info("TC_01_Create_A_Manufacturing_Order - Step 01:  Navigate to 'Manufacturing' feature");
		homePage.clickOnAListBox("Manufacturing");
		manufacturingPage = PageGenerateManager.getManufacturingPage(driver);

		log.info("TC_01_Create_A_Manufacturing_Order - Step 02: Click 'Create' button");
		manufacturingPage.clickOnButton(driver, "Create");

		log.info("TC_01_Create_A_Manufacturing_Order - Step 03: Enter and select a created product name");
		manufacturingPage.enterValueIntoTextboxFromDropdownByLabel(driver, "Product", productName);

		log.info("TC_01_Create_A_Manufacturing_Order - Step 04: Click 'Confirm' button");
		manufacturingPage.clickOnButton(driver, "Confirm");

		log.info("TC_01_Create_A_Manufacturing_Order - Step 05: Update quantity of product");
		manufacturingPage.enterValueIntoTextboxFromDropdownByLabel(driver, "Quantity",
				driverData.getQuantityOnMacfacturingOrderPage());

		log.info("TC_01_Create_A_Manufacturing_Order - Step 06: Add a product in component table");
		...
		log.info("TC_01_Create_A_Manufacturing_Order - Step 07: Click 'SAVE' button");
		manufacturingPage.clickOnButton(driver, "Save");

		log.info("TC_01_Create_A_Manufacturing_Order - Step 08: Get information on 'Manufacturing order detail' page");
		...
		log.info("TC_01_Create_A_Manufacturing_Order - Step 09: Click 'Mark As Done' button");
		manufacturingPage.clickOnButton(driver, "Mark as Done");

		log.info("TC_01_Create_A_Manufacturing_Order - Step 10: Click 'Manufacturing' main menu");
		manufacturingPage.clickOnMenuBrandOnTheTopBar(driver, "Manufacturing");

		log.info("TC_01_Create_A_Manufacturing_Order - Step 11: Search " + manufacturingOderId);
		manufacturingPage.searchManufactoringOrderInSearchTextbox(manufacturingOderId);

		log.info("TC_01_Create_A_Manufacturing_Order - Step 12: Verify information of productName, 'Quantity', 'UoM', 'Status'");
